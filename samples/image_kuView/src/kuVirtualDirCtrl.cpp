	...
	if(mFrame->mIsThumbnail && GetFilePath(true,true)==wxEmptyString) {
        ...
        if(nid.IsOk())
            for(int i=0; i<wxGetApp().mOptions.mLoadSiblings[0]; i++) {
                ...
                wxGetApp().mLoader->Append(GetFilePath(true,true,nid), true, size);
            }
        ...
        if(pid.IsOk())
            for(int i=0; i<wxGetApp().mOptions.mLoadSiblings[1]; i++) {
                ...
                wxGetApp().mLoader->Append(GetFilePath(true,true,pid), true, size);
            }
        if(nid.IsOk())
...
void kuVirtualDirCtrl::SetRoot(wxString archive) {
	...
	AddRoot(archive.AfterLast(wxFileName::GetPathSeparator()),-1,-1,new kuVirtualDirData(archive,true));
...
void kuVirtualDirCtrl::EnumerateArchive(wxString archive) {
    ...
    if(IsImageFile(archive)) {   // for file like xpm.gz
        AppendItem(GetRootItem(),archive.AfterLast(sep),wxFileIconsTable::file,-1,new kuVirtualDirData(archive,false));
        mFrame->SetStatusText(wxEmptyString);
        return;
    }
        ...
        while(entry = arc->GetNextEntry()) {
            ...
            while((pdir=text.BeforeFirst(sep))!=wxEmptyString) {   // search down
                ...
                if(!pid.IsOk()) {   // create parent
                    pid = AppendItem(aid,pdir,wxFileIconsTable::folder,-1,new kuVirtualDirData(pdir,true));
                }
                aid = pid;
                text = text.AfterFirst(sep);
            }
            if(IsImageFile(text))   // skip non-image file
                AppendItem(pid,text,wxFileIconsTable::file,-1,new kuVirtualDirData(name,false));
        }
    }
    mFrame->SetStatusText(wxEmptyString);
...
/*
	...
	wxTreeItemId      pid,aid;
	mFrame->SetStatusText(STRING_INFO_ENUMERATE_ARCHIVE);
	while((entry=archive.GetNextEntry())!=NULL) {   // enumerate files and dirs in archive
		if(entry->IsDir())   continue;
		text=name=entry->GetName();
		aid=pid=GetRootItem();   // keep parent of parent
		while((pdir=text.BeforeFirst(sep))!=wxEmptyString) {   // search down
...
bool kuVirtualDirCtrl::IsImageFile(wxString& filename) {
...
bool kuVirtualDirCtrl::IsDir(wxTreeItemId id) {
	if(((kuVirtualDirData*)GetItemData(id))->mIsDir)   return true;
	else   return false;
}
...
wxString kuVirtualDirCtrl::GetFilePath(bool isurl, bool fileonly, wxTreeItemId id) {
	...
	if(!id.IsOk() || (fileonly&&IsDir(id)))   return wxEmptyString;
        ...
        wxString filename=((kuVirtualDirData*)GetItemData(id))->mFilename;
...
wxArrayString* kuVirtualDirCtrl::EnumerateChildren(wxString dirname) {
    ...
    if(GetFilePath(true,true)==wxEmptyString) {   // if it is directory
        ...
        while(true) {
            ...
            if(!id.IsOk())   break;
            // if child is a file
            if(!((kuVirtualDirData*)GetItemData(id))->mIsDir)
...
// -------- kuVirtualDirData --------
kuVirtualDirCtrl::kuVirtualDirData::kuVirtualDirData(wxString& filename, bool isdir)
	:mFilename(filename), mIsDir(isdir) {
}
